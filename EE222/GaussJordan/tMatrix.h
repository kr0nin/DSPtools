...
tMatrix addition(tMatrix m1,tMatrix m2){
    ...
    if ((m1.rows != m2.rows) || (m1.cols != m2.cols)){
        printf("Error! Invalid Dimensions for addition");
        exit(1);
    }
...
tMatrix subtraction(tMatrix m1,tMatrix m2){
    ...
    int i,j;
    if ((m1.rows != m2.rows) || (m1.cols != m2.cols)){
        printf("Error! Invalid Dimensions for subtraction");
        exit(1);
    }
    m3.rows = m1.rows; m3.cols = m1.cols;
    m3 = allocate(m3);
...
tMatrix multiply(tMatrix m1,tMatrix m2){
    ...
    int r,c,p;
    if (m1.cols != m2.rows){
        printf("ERROR! multiplication issues");
        return m3;
    }
    else{
        m3.rows = m1.rows;
