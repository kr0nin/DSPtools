...
tMatrix_zp DFT(tMatrix_zc orgmat, tMatrix_zc unitmat){
    ...
    //actual DFT
    for (k = 0; k < orgmat.cols; k++){
        for (n = 0; n < orgmat.cols; n++){
            tempsum = tConvert_cp(tAdd(tConvert_pc(tempsum),tMultiply(orgmat.data[n][1],tConvert_pc(tComplexPower(twimat.data[k][1],n)))));
        }
